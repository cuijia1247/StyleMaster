...
# -*- coding: utf-8 -*-
"""
基于 ImageNet 预训练 ResNet50 的风格分类测试脚本。

用法:
    python resnet50_test.py --benchmark_all
    python resnet50_test.py --data_root /mnt/codes/data/style/Painting91 --num_classes 13 --epochs 10 --runs 5
"""
...
def find_local_resnet50_checkpoint(checkpoint_dir: str) -> str | None:
    """在 pretrainModels/hub/checkpoints 下查找 ResNet50 权重文件。"""
    if not os.path.isdir(checkpoint_dir):
        return None
    matches = sorted(glob.glob(os.path.join(checkpoint_dir, "resnet50*.pth")))
...
def load_pretrained_resnet50(device: torch.device, verbose: bool = True) -> models.ResNet:
    """
    ...
    local_ckpt = find_local_resnet50_checkpoint(checkpoint_dir)
    if local_ckpt is not None:
        if verbose:
            print(f"[ResNet50] 使用本地预训练权重: {local_ckpt}")
        model = models.resnet50(weights=None)
        model.load_state_dict(_load_state_dict(local_ckpt))
    else:
        if verbose:
            print(f"[ResNet50] 本地未找到权重，下载至: {pretrain_root}")
        model = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1)

    return model.to(device)


def build_resnet50_classifier(num_classes: int, device: torch.device) -> models.ResNet:
    """
    构建 ResNet50 分类器：
    - 主干网络冻结；
    - 替换 fc 为 num_classes 输出；
    - 仅微调 fc 层。
    """
    model = load_pretrained_resnet50(device, verbose=False)
...
def _set_backbone_eval(model: models.ResNet) -> None:
    """冻结主干时保持 BatchNorm 处于 eval，仅 fc 参与训练。"""
    model.eval()
    model.fc.train()
...
def train_single_run(
    ...
    model = build_resnet50_classifier(num_classes, device)
...
def run_dataset_benchmark(
    ...
    load_pretrained_resnet50(device, verbose=True)
...
def write_result_markdown(
    ...
    lines = [
        "# ResNet50 多数据集多次实验",
        "",
        f"## ResNet50 benchmark ({dataset_names}) (epochs={epochs}, runs={runs}) — {timestamp}",
        "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ResNet50 预训练风格分类测试")
    parser.add_argument(
        "--benchmark_all",
        action="store_true",
    ...
    parser.add_argument("--epochs", type=int, default=10, help="微调 epoch 数")
    ...
    parser.add_argument(
        "--result_md",
        type=str,
        default=os.path.join(get_project_root(), "ieee_access_paperdata", "resnet50_multiple.md"),
...
def main() -> None:
    ...
    # 默认跑五个 benchmark 数据集；指定 --data_root 时仅跑单个数据集
