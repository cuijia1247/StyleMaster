...
# -*- coding: utf-8 -*-
"""
基于 ImageNet 预训练 VGG16 的风格分类测试脚本。

用法:
    python vgg16_test.py --benchmark_all
    python vgg16_test.py --data_root /mnt/codes/data/style/Painting91 --num_classes 13
"""
...
def find_local_vgg16_checkpoint(checkpoint_dir: str) -> str | None:
    """在 pretrainModels/hub/checkpoints 下查找 VGG16 权重文件。"""
    if not os.path.isdir(checkpoint_dir):
        return None
    matches = sorted(glob.glob(os.path.join(checkpoint_dir, "vgg16*.pth")))
...
def load_pretrained_vgg16(device: torch.device, verbose: bool = True) -> models.VGG:
    """
    加载 ImageNet 预训练 VGG16：
    ...
    local_ckpt = find_local_vgg16_checkpoint(checkpoint_dir)
    if local_ckpt is not None:
        if verbose:
            print(f"[VGG16] 使用本地预训练权重: {local_ckpt}")
        model = models.vgg16(weights=None)
        model.load_state_dict(_load_state_dict(local_ckpt))
    else:
        if verbose:
            print(f"[VGG16] 本地未找到权重，下载至: {pretrain_root}")
        model = models.vgg16(weights=models.VGG16_Weights.IMAGENET1K_V1)

    return model.to(device)


def build_vgg16_classifier(num_classes: int, device: torch.device) -> models.VGG:
    """
    构建 VGG16 分类器：
    - 卷积特征层冻结；
    - 替换最后一层全连接为 num_classes 输出；
    - 仅微调 classifier 部分。
    """
    model = load_pretrained_vgg16(device, verbose=False)
...
def train_single_run(
    ...
    model = build_vgg16_classifier(num_classes, device)
...
def run_dataset_benchmark(
    ...
    load_pretrained_vgg16(device, verbose=True)
...
def write_result_markdown(
    ...
    lines = [
        "# VGG16 多数据集多次实验",
        "",
        f"## VGG16 benchmark ({dataset_names}) (epochs={epochs}, runs={runs}) — {timestamp}",
        "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="VGG16 预训练风格分类测试")
    parser.add_argument(
        "--benchmark_all",
        action="store_true",
    ...
    parser.add_argument("--epochs", type=int, default=1, help="微调 epoch 数")
    parser.add_argument("--runs", type=int, default=10, help="重复实验次数，报告 mean±std")
    ...
    parser.add_argument(
        "--result_md",
        type=str,
        default=os.path.join(get_project_root(), "ieee_access_paperdata", "vgg16_multiple.md"),
