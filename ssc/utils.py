...
def criterion(x, y, lmbd = 5e-3, u = 1, v= 1, epsilon = 1e-3, device=torch.device('cuda')): #the base loss computation
    bs = x.size(0)
    emb = x.size(1)
    ...
    cross_loss = (crossCorMat*lmbd - torch.eye(emb, device=device)*lmbd).pow(2).sum()
...
def get_ssc_transforms(size, mean, std): #special for SSC training
    transformT = tr.Compose([
        transforms.ToTensor(),
        tr.RandomResizedCrop(size=size, scale=(0.5,0.7), ratio=(3 / 4, 4 / 3)),
