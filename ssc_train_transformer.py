...
from ssc.classifier_enhance import Classifier, EfficientClassifier  # 风格分类头（双路直接融合版）
...
def parameter_load():
    ...
    image_size = 112                                    # 随机裁剪子图的边长（像素），从64提升到112，插值倍率从3.5x降到2x，保留更多纹理细节
    # classfier_iteration = 180 # best
    classfier_iteration = 80                             # 每次触发分类器训练时的迭代轮数（前50轮已基本收敛，80轮足够）
    classifier_lr = 0.0001                               # 分类器学习率
    # classifier_training_gap = 30 # best
    # classifier_test_gap = 30 # best
    classifier_training_gap = 10                         # 每隔多少个 epoch 触发一次分类器训练
