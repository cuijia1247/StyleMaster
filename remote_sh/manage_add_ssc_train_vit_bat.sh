...
# add 版 SSC 批量训练进程管理脚本
# 用法: ./remote_sh/manage_add_ssc_train_vit_bat.sh {start|stop|restart|status|tail|logs|result}
...
start_training() {
    ...
    echo -e "${GREEN}启动 add 版批量训练...${NC}"
    ./remote_sh/run_add_ssc_train_vit_bat.sh
}
...
check_status() {
    if [ ! -f "$PID_FILE" ]; then
        echo -e "${RED}add 批量训练未运行${NC}"
        return 1
    fi

    PID=$(cat "$PID_FILE")
    if ps -p "$PID" > /dev/null 2>&1; then
        echo -e "${GREEN}add 批量训练正在运行${NC}"
        echo "PID: $PID"
        echo ""
        echo "进程信息:"
        ps -fp "$PID"
        ...
        echo ""
        # 优先找主日志，其次找单数据集日志
        LATEST=$(ls -t "$LOG_DIR"/add_bat_main_*.log \
                        "$LOG_DIR"/ssc-add-bat-*.log 2>/dev/null | head -1)
        if [ -n "$LATEST" ]; then
...
tail_log() {
    LATEST=$(ls -t "$LOG_DIR"/add_bat_main_*.log \
                    "$LOG_DIR"/ssc-add-bat-*.log 2>/dev/null | head -1)
    if [ -z "$LATEST" ]; then
        echo -e "${RED}未找到 add 批量训练日志文件${NC}"
        return 1
    fi
...
list_logs() {
    echo -e "${GREEN}add 批量训练日志列表 (add_bat_main_* / ssc-add-bat-*):${NC}"
    echo ""
    if [ -d "$LOG_DIR" ]; then
        ls -lht "$LOG_DIR"/add_bat_main_*.log \
                "$LOG_DIR"/ssc-add-bat-*.log 2>/dev/null \
