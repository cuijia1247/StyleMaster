...
# DenseNet169-6ch add 版 SSC 批量多数据集训练脚本
# 依据: ssc_train_densnet169_add.py（EfficientRWPClassifier + 内存 DenseNet GAP 缓存）
# 数据集: Painting91, Pandora, AVAstyle, Arch, FashionStyle14, webstyle/subImages
# 每数据集重复 RUNS=3 次，记录准确率与 mean±std → remote_sh/densenet_batch_result.md
# 用法: 从项目根目录执行 ./remote_sh/run_add_ssc_train_densenet_bat.sh
...
RUNNER_SCRIPT="remote_sh/_add_densenet169_bat_runner.py"

if [ -f "$PID_FILE" ]; then
    OLD_PID=$(cat "$PID_FILE")
    if ps -p "$OLD_PID" > /dev/null 2>&1; then
        echo "DenseNet 批量训练进程已在运行 (PID: $OLD_PID)"
        echo "如需重新启动，请先执行: ./remote_sh/manage_add_ssc_train_densenet_bat.sh stop"
        exit 1
    else
        rm -f "$PID_FILE"
    fi
...
LOG_FILE="$LOG_DIR/densenet_bat_main_${TIMESTAMP}.log"

echo "=========================================="
echo "启动 DenseNet169 add 版 SSC 批量训练 (6 数据集 × 3 次)"
echo "主日志: $LOG_FILE"
...
echo ""
echo "管理命令:"
echo "  ./remote_sh/manage_add_ssc_train_densenet_bat.sh {status|tail|stop|result}"
echo ""

sleep 3
if ps -p "$TRAIN_PID" > /dev/null 2>&1; then
    echo "√ 进程已启动，可安全断开 SSH"
else
