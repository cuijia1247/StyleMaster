...
# Transformer 超参数网格搜索后台运行脚本
# Author: cuijia1247
# Date: 2026-03-29
# 功能：使用 nohup 在后台运行参数优化，断开 SSH 后仍可继续
# 用法：从项目根目录执行 ./remote_sh/run_transformer_param_optim.sh
...
# 创建结果与日志目录
mkdir -p param_optim/transformer/logs

# 生成时间戳
TIMESTAMP=$(date +"%Y-%m-%d-%H-%M-%S")
NOHUP_LOG="param_optim/transformer/nohup_${TIMESTAMP}.log"
...
# 启动后台参数搜索
echo "=========================================="
echo "启动 Transformer 超参数网格搜索..."
echo "nohup 日志: $NOHUP_LOG"
echo "各组合详细日志: param_optim/transformer/logs/"
echo "汇总结果 CSV: param_optim/transformer/grid_search_results.csv"
echo "开始时间: $(date)"
echo "=========================================="

nohup python param_optim/transformer_param_optim.py > "$NOHUP_LOG" 2>&1 &
OPTIM_PID=$!
...
echo "  查看汇总结果: cat param_optim/transformer/grid_search_results.csv"
