...
# ResNet50 批量训练进程管理脚本
# 用法: ./remote_sh/manage_ssc_train_resnet_bat.sh {start|stop|restart|status|tail|logs|result}
...
check_status() {
    ...
    if ps -p "$PID" > /dev/null 2>&1; then
        ...
        echo ""
        # 最新主日志
        LATEST=$(ls -t "$LOG_DIR"/bat_main_*.log "$LOG_DIR"/bat-*.log 2>/dev/null | head -1)
        if [ -n "$LATEST" ]; then
...
tail_log() {
    # 优先找主进程日志（bat_main_），其次找单数据集日志（bat-）
    LATEST=$(ls -t "$LOG_DIR"/bat_main_*.log "$LOG_DIR"/bat-*.log 2>/dev/null | head -1)
    if [ -z "$LATEST" ]; then
...
list_logs() {
    echo -e "${GREEN}批量训练日志列表 (bat_main_* / bat-*):${NC}"
    echo ""
    if [ -d "$LOG_DIR" ]; then
        ls -lht "$LOG_DIR"/bat_main_*.log "$LOG_DIR"/bat-*.log 2>/dev/null \
