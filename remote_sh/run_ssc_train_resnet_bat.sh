...
# ResNet50 批量多数据集训练脚本
# 每次启动会用下方 PYEOF 重新生成 remote_sh/_resnet50_bat_runner.py，避免磁盘上残留旧版 runner。
# 修改批量逻辑：请编辑本文件中的 PYEOF 段，并同步提交 _resnet50_bat_runner.py（内容应与 PYEOF 一致，便于 IDE 浏览）。
# 训练超参来自 ssc_train_resnet_copy.parameter_load()，与单库脚本一致。
...
# 每次启动覆盖生成 runner（与 ssc_train_resnet_copy 逻辑分离；超参仍由 copy.parameter_load 决定）
echo "重新生成 $RUNNER_SCRIPT ..."
cat > "$SCRIPT_DIR/$RUNNER_SCRIPT" << 'PYEOF'
...
echo "=========================================="
echo "启动 ResNet50 批量训练（6 数据集，每库 5 轮 best → mean±std）"
echo "Runner: $RUNNER_SCRIPT（已重新生成）"
echo "主日志: $LOG_FILE"
