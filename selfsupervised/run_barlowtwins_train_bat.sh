#!/usr/bin/env bash
#
# Barlow Twins 六数据集批量训练（每库跑 3 遍，mean±std 写入 selfsupervised/barlowtwins_result.md）
# 适合 SSH 离线：默认 nohup 后台，断开终端仍继续。
#
# 用法（均在项目根目录执行）::
#   ./selfsupervised/run_barlowtwins_train_bat.sh              # 后台 + nohup
#   ./selfsupervised/run_barlowtwins_train_bat.sh fg           # 前台（调试，Ctrl+C 结束）
#
...
LOG_FILE="$LOG_DIR/barlowtwins_bat_${TIMESTAMP}.log"
RESULT_MD="$SELF_DIR/barlowtwins_result.md"
