"""
基于 barlowtwins/barlow.py 中的 BarlowTwins 与 barlow_loss_fun：
1) 双视图 Barlow Twins 自监督预训练；2) 冻结 encoder，线性分类头在 test 上报告最佳准确率。

六数据集、三轮测试与 selfsupervised/simclr_train.py 对齐，结果追加写入 selfsupervised/barlowtwins_result.md。

用法::
    python selfsupervised/barlowtwins_train.py --benchmark_all
...
DEFAULT_RESULT_MD = os.path.join(_ROOT, "selfsupervised", "barlowtwins_result.md")
...
def append_benchmark_markdown(
    ...
    lines = [
        "",
        f"## Barlow Twins 六数据集 (backbone=resnet50, run={num_runs}) — {ts}",
        "",
...
def train_barlow_once(args: Any, logger: logging.Logger) -> float:
    ...
    for epoch in range(1, args.pretrain_epochs + 1):
        ...
        if epoch == 1 or epoch % 5 == 0 or epoch == args.pretrain_epochs:
            logger.info(
                "[Barlow pretrain] epoch %d/%d loss=%.6f",
                epoch,
                args.pretrain_epochs,
                mean_loss,
            )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Barlow Twins（barlowtwins/barlow.py）+ 线性探针，六数据集风格分类"
    )
...
def main() -> None:
    ...
    if args.benchmark_all:
        ...
        for rel, n_cls, label in BENCHMARK_DATASETS:
            ...
            for r in range(1, args.num_runs + 1):
                ...
                try:
                    best = train_barlow_once(args, logger)
                    accs.append(best)
    ...
    for r in range(1, args.num_runs + 1):
        ...
        logger = _make_logger(log_path)
        best_acc = train_barlow_once(args, logger)
