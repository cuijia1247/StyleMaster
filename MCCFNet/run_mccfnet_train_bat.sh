#!/usr/bin/env bash
#
# MCCFNet (mccfnet_train.py) 五数据集批量训练
# 数据集: Painting91, Pandora, ArtBench, FashionStyle14, Arch
# 默认参数沿用 mccfnet_train.py，每库 runs=3，四项指标
# 结果合并写入 ieee_access_paperdata/MCCFNet_multiple.md（格式对齐 BarlowTwins_multiple.md）
#
# 用法（均在项目根目录执行）::
#   ./MCCFNet/run_mccfnet_train_bat.sh              # 后台 + nohup
#   ./MCCFNet/run_mccfnet_train_bat.sh fg           # 前台（调试）
#
# 管理: ./MCCFNet/manage_mccfnet_train_bat.sh {start|stop|status|tail|result|…}
...
RESULT_MD="$ROOT/ieee_access_paperdata/MCCFNet_multiple.md"
...
TIMESTAMP="${MCCF_BAT_TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"
LOG_FILE="${MCCF_BAT_LOG:-$LOG_DIR/mccfnet_bat_${TIMESTAMP}.log}"
PARTIAL_DIR="${MCCF_BAT_PARTIAL:-$LOG_DIR/mccfnet_partials_${TIMESTAMP}}"
...
lines = [
    "# MCCFNet 多数据集多次实验",
    "",
    f"## MCCFNet benchmark ({', '.join(dataset_order)}) "
    f"(epochs={epochs}, runs={runs}) — {datetime.now():%Y-%m-%d %H:%M:%S}",
    "",
    f"_data_base=`{data_base}`_",
    "",
    f"_命令: `./MCCFNet/run_mccfnet_train_bat.sh` → `mccfnet_train.py` × {len(dataset_order)} 数据集_",
    "",
]
...
run_batch() {
  echo "ROOT=$ROOT"
  echo "训练脚本: $ROOT/MCCFNet/mccfnet_train.py"
  echo "日志: $LOG_FILE"
...
run_background() {
  ...
  nohup env \
    MCCF_BAT_LOG="$LOG_FILE" \
    MCCF_BAT_PARTIAL="$PARTIAL_DIR" \
    MCCF_BAT_TIMESTAMP="$TIMESTAMP" \
    "$SELF_DIR/run_mccfnet_train_bat.sh" _batch >>"$LOG_FILE" 2>&1 &
  echo $! >"$PID_FILE"
...
case "${1:-}" in
  ...
  _batch)
    LOG_FILE="${MCCF_BAT_LOG:-$LOG_FILE}"
    PARTIAL_DIR="${MCCF_BAT_PARTIAL:-$PARTIAL_DIR}"
    run_batch
    ;;
  *) run_background ;;
esac
