...
def calculate_similarities(model, transform, dataSource):
    """
    计算所有图像的相似度
    返回: 
    ...
    for root, dirs, files in os.walk(dataSource):
        for file in files:
            ...
            try:
                ...
                # 计算相似度
                similarity = F.cosine_similarity(view1, view2).item()
...
def clean_images_by_similarity(model_path, dataSource, output_root, thresholds):
    """主清洗函数"""
    # 加载模型
    base_model = torch.load(model_path + 'base-best.pth', map_location=device)
    ...
    # 计算所有图像的相似度
    similarity_dict, top_5, bottom_5 = calculate_similarities(base_model, transform, dataSource)
...
if __name__ == '__main__':
    dataSource = '/home/huangrui/Codes/SubStyleClassfication/merged_data/test'
    model_path = '/home/huangrui/Codes/SubStyleClassfication/model/pandora-SSR-resnet50-2025-05-09-15-20-31-SSC-'
